...
if (isset($_POST['submitted'])) { 
    ...
    if($_POST['Ano'] == "0"){
         ...
         echo " <div class=\"alert alert-danger\">
                  <strong> <h3>Por favor escolha um ano </h3></strong> 
          <br>  Você precisa escolher um ano de referencia para esse lançamento.

          </div>";

          echo "<input class='btn btn-lg btn-info type='submit' value='Voltar' onclick='history.back(-1)' />   ";
    ...
    if($_POST['Mes'] == "0"){
         ...
         echo " <div class=\"alert alert-danger\">
                  <strong> <h3>Por favor escolha um Mês </h3></strong> 
          <br>  Você precisa escolher um Mês de referencia para esse lançamento.
    ...
    if( $_POST['selectUsuario'] == "0"){

         echo " <div class=\"alert alert-danger\">
                  <strong> <h3>Por favor escolha um Campo </h3></strong> 
          <br>  Você precisa escolher um campo para esse lançamento.
    ...
    //Data de baixa
    $DataBaixa = date('Y-m-d', strtotime(str_replace('/', '-', $_POST['dtBaixa'])));
...
$sql = "UPDATE `lancamentosbancarios` SET  TipoLancamento = 'Identificado'
    ...
    if(verificaLancamento(0,$data_referencia,$idUsuario)){
        ...
        Logger("# O usuario {$_SESSION['nome']}({$_SESSION['idlogado']}) fez uma IDENTIFICACAO do Lancamento de ID:: {$id} ");
        Logger("# Valor :: {$valor_cobrado} ");
        Logger("# Referente ao mes :: {$data_referencia}" );
...
<?php
$tituloPrincipal = "Identificação de depósitos";
$tituloSecondario = "Identificação";
        ...
        echo "<select  data-placeholder=\"Escolha um usuário... \" id=\"selectUsuario\"    \" name=\"selectUsuario\" class=\"form-control\" class=\"chosen-select\" 
         \>";
            ...
            $resultSelect = $db->query("
                                            ...
                                            where 
                                            u.idTipoUsuario = 6   
                                            or u.idTipoUsuario = 7 
                                            or u.idTipoUsuario = 9    
                                            or u.idTipoUsuario = 4                                          
                                        union
...
<div class="row">
    ...
    <div class="col-lg-3">
        <div class="form-group">
            ...
            <label>Url</label>
            <input type="text" name="urlFile"  class="form-control"  value=''/>
...
include "logger.php";
Logger("{$_SESSION['nome']} [ID::{$_SESSION['idlogado']}] acessou IDENTIFICACAO de LANC de ID:: {$fID}   Valor:: {$row['Valor']}  NmDocto :: {$row['NumeroDocumento']} ");
#Logger("Campo referente ao Lancamento {$row['Nome']} [ID::$row['id']] ");
