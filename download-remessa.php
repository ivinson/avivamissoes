...
$DATA['DIA'] = gmdate("dd",$timestamp);
$DATA['MES'] = gmdate("mm",$timestamp);
$DATA['ANO'] = gmdate("yy",$timestamp);
...
$result = $db->query("
	...
	$dt_vencimento = explode('/',$row['DataVenc']);
	 //$ddmmyy =  gmdate("d/m/Y", $dt_vencimento);
	 //echo "gmtade is " . $ddmmyy;
	
	 //die();
	//print_r($dt_vencimento);




	$dt_vencimento = $dt_vencimento[0].$dt_vencimento[1] . substr($dt_vencimento[2],2,4);	

	$vencimento = $dt_vencimento;	
	echo $vencimento . " -->>  {$row['DataVenc']}<br>";
...
$conteudo .= limit($cliente['nosso_numero'],25);              //      USO / IDENT. DO TÍTULO NA EMPRESA NOTA 2        038 062         X(25)
...
$conteudo .= formatarNossoNumero($cliente['nosso_numero']);  
...
//echo  "::::  " . limit($cliente['vencimento'],6) . ":::::::<br>";
